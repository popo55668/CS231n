...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(len(hidden_dims)):
            ...
            self.params['W' + str(i+1)] = weight_scale * \
                    np.random.randn(prev_dim, cur_dim)
            self.params['b' + str(i+1)] = np.zeros(cur_dim)

            if self.normalization != None:
                self.params['gamma' + str(i+1)] = np.ones(cur_dim)
                self.params['beta' + str(i+1)] = np.zeros(cur_dim)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
            ...
            if i != self.num_layers - 1:
                if self.normalization == 'batchnorm':
                    ...
                    beta = self.params['beta' + str(i+1)]
                    bn_param = self.bn_params[i]
                    scores, cache = layernorm_forward(scores, gamma, beta, bn_param)
                    caches['bn' + str(i+1)] = cache

                scores, cache = relu_forward(scores)
                caches['cache_relu' + str(i+1)] = cache

                if self.use_dropout:
                    scores, caches['dropout' + str(i+1)] =\
        ...
        for i in reversed(range(self.num_layers)):
            idx = str(i+1)
            cache = caches['cache' + idx]
            dx, grads['W' + idx], grads['b' + idx] = affine_backward(dx, cache)
            grads['W' + idx] += self.reg * self.params['W' + idx]

            if i != 0:
                if self.use_dropout:
                    cache = caches['dropout' + str(i)]
                    dx = dropout_backward(dx, cache)

                cache = caches['cache_relu' + str(i)]
                dx = relu_backward(dx, cache)

                if self.normalization == 'batchnorm':
                    cache = caches['bn' + str(i)]
